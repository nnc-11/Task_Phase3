...
Không tạo mutation mới chỉ để test. Chọn một `PutMetricAlarm` hoặc `PutRule` vốn đã nằm trong approved foundation apply và lưu:

1. baseline/pre-state và saved-plan hash;
2. raw CloudTrail event trong archive;
3. output redacted từ `Export-M12CloudTrailEvidence.ps1 -EvidenceProfile ConfigChange`;
4. post-state bằng `describe-alarms` hoặc `describe-rule`;
5. diff chỉ rõ trường và giá trị đã đổi.

PASS khi `changeParameters`, actor/session, UTC, request ID và post-state khớp plan. Không xuất policy document, request body tùy ý, secret/token/password hoặc production object content.

## Heartbeat

Không cố ý tắt trail. Xác minh Lambda log `PASS`, schedule, exact rule pattern/target và source/eventSource semantics, routers, alarms, confirmed subscriptions trên primary/global/fallback, EKS audit flag, Versioning/Object Lock/lifecycle/encryption/public block và thresholds. Digest dùng ngưỡng riêng 90 phút vì giao chậm hơn log files.

Test đường dự phòng phải dùng `forceAlertTest=true` theo HD trong approved window để chứng minh Lambda publish primary/global độc lập; không xóa hoặc sửa topic production. PASS khi payload trả về đủ hai region, không có delivery failure và người trực nhận hai test messages. Hai alarm phải có chính xác primary và fallback cùng region trong `AlarmActions`; dùng bounded `set-alarm-state` trên Errors alarm để chứng minh cả primary và fallback nhận cùng test alarm, không disable action hoặc sửa threshold.

## Approved-change alert

Trước apply, change ID phải ghi Git SHA, saved-plan hash, principal, UTC window và danh sách g7/g8 action dự kiến. Mọi CRITICAL alert trong window vẫn phải được nhận và đối chiếu; PASS khi người trực gắn verdict `EXPECTED/<change-id>` hoặc mở incident nếu không khớp. Không tắt alarm/topic và không thêm suppression để giảm nhiễu.
